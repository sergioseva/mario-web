...
import model.librosmario.*;

public class PedidoPrinter {

	//static Logger logger = Logger.getLogger(PedidoPrinter.class);
	
	public static void ImprimirPedido(Pedido p) throws Exception {
		java.sql.Connection c=null;
		//DOMConfigurator.configure("log4j.xml");
		try {
		      Map parameters = new HashMap();
		      c=Sistema.getConnection();
		      parameters.put("nroPedido", p.getPe_pedido_k());		      
		      JasperReport report = JasperCompileManager.compileReport(
		    	  Sistema.getInstance().getPathReports() + "pedido_cliente.jrxml");
		      ...
		      Sistema.logger.error("error imprimiendo pedido del cliente:" + e.getStackTrace());
		      throw e;
		    }
		    finally {
		      /*
